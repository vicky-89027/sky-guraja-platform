...
export const SkyLogo: React.FC<SkyLogoProps> = ({
  ...
  const Emblem = (
    <svg
      ...
      className="flex-shrink-0 drop-shadow-[0_0_12px_rgba(245,158,11,0.4)]"
    >
      <defs>
        ...
        <linearGradient id="featherOuter" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      {/* Letter 'K' - Backbone and dynamic arms */}
      <path
        d="M 88 50 L 88 152"
        stroke="url(#goldGrad1)"
        strokeWidth="13"
      ...
      {/* Letter 'Y' - Intertwined Graceful Curve */}
      <path
      ...
      <g transform="translate(116, 12) rotate(15)">
        ...
        <path
          ...
          fill="url(#featherOuter)"
          stroke="url(#goldGrad1)"
          strokeWidth="1.5"
        />
        {/* Intermediate Turquoise Ring */}
        <ellipse cx="22" cy="24" rx="12" ry="16" fill="#0284C7" />
        {/* Peacock Eye Inner Blue & Gold Core */}
        <ellipse cx="22" cy="24" rx="7" ry="9" fill="url(#featherEye)" />
  ...
  // Full Stacked Display (Grand Hero & Mobile Landing)
  if (variant === 'full') {
    return (
      <div className={`flex flex-col items-center text-center space-y-3 ${className}`}>
        <div className="relative group">
          <div className="absolute -inset-2 bg-amber-500/20 rounded-full blur-xl group-hover:bg-amber-500/30 transition-all" />
        ...
        <div>
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-4 mt-3 text-[9px] sm:text-[10px] text-amber-200/80 font-mono tracking-wider uppercase">
            <span>Unity</span>
  ...
  // Horizontal Format (Default Header)
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {Emblem}
      <div className="text-left">
